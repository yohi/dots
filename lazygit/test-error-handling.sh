...
if grep -q "set -o pipefail" ai-commit-generator.sh; then
...
if grep -q "set -o pipefail" parse-ai-output.sh; then
...
if grep -q "timeout.*TIMEOUT_SECONDS" ai-commit-generator.sh; then
...
if grep -q "AI tool timed out" ai-commit-generator.sh; then
...
OUTPUT=$(echo -e "feat: add feature\nfix: fix bug" | ./parse-ai-output.sh 2>&1)
...
ERROR_COUNT=$(grep -c "Suggestion:" ai-commit-generator.sh || true)
